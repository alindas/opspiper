...
export default function Deploy() {
  ...
  // 上传或安装时的进度
  const [speedProgress, setSpeedProgress] = useState('0.0');
  ...
  */
  ...
  const handleInstallAndUpgradeXML = (form: any, comment: string) => {
    ...
    const handleSuccess = (xhr: XMLHttpRequest) => {
      // 返回的是字符串，表示操作成功
    ...
    const handleFail = (status: number, message: string) => {
      notification.error({
        message: '请求失败',
        description: message,
        duration: 1.5
    ...
    const handleProgress = (evt: ProgressEvent<EventTarget>, xhr: XMLHttpRequest) => {
      setSpeedProgress((evt.loaded / evt.total * 100).toFixed(1));
    }

    uploadRequestWithProcess({
      url: `http://${Server.host}:${Server.port}/api/${handleType.buttonType}${handleType.buttonType === 'upgrade' ? `?name=${name}` : ''}`,
      header: headerConfig,
      data: file,
